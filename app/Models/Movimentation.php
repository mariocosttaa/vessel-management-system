...
class Movimentation extends Model
{
    ...
    {
        return $this->belongsTo(MovimentationCategory::class, 'category_id');
    }
    ...
    {
        return $this->belongsTo(RecurringMovimentation::class);
    }
    ...
    {
        return $this->hasMany(MovimentationFile::class, 'transaction_id');
    }
    ...
    private static function generateTransactionNumber(): string
    {
        ...
        // For SQLite compatibility, we'll get all and sort in PHP
        $lastTransaction = self::where('transaction_number', 'like', "TRX{$year}%")
            ->get()
            ->sortByDesc(function ($transaction) {
                return (int) substr($transaction->transaction_number, -6);
            })
            ->first();

        // Start from the next number after the last one, or 1 if none exists
        ...
        // This handles race conditions and ensures we get a unique number
        $maxAttempts = 1000;
        $attempts    = 0;

        do {
            $transactionNumber = sprintf('TRX%s%06d', $year, $nextNumber);

            // Use a direct database query to check existence (more reliable)
        ...
        $timestampNumber = (int) substr(time(), -6);
        return sprintf('TRX%s%06d', $year, $timestampNumber);
    }
    ...
    {
        return EasyHashAction::encode($this->id, 'movimentation-id');
    }
    ...
    public function resolveRouteBinding($value, $field = null)
    {
        ...
        $decoded = EasyHashAction::decode($value, 'movimentation-id');
