...
const form = useForm({
    ...
    description: '',
    is_default: false,
    items: [] as DistributionProfileItem[],
});
...
const handleSaveItem = (item: DistributionProfileItem) => {
    if (editingItemIndex.value !== null) {
        // Edit existing item - preserve order_index
        form.items[editingItemIndex.value] = {
            ...item,
            order_index: form.items[editingItemIndex.value].order_index, // Preserve original order_index
        };
    } else {
        // Add new item
...
const handleSubmit = () => {
    form.post(`/panel/${getCurrentVesselId()}/marea-distribution-profiles`, {
        onSuccess: () => {
...
<template>
    <Head title="Create Distribution Profile" />

    <VesselLayout :breadcrumbs="[
        { title: 'Distribution Profiles', href: `/panel/${getCurrentVesselId()}/marea-distribution-profiles` },
        { title: 'Create Profile', href: `/panel/${getCurrentVesselId()}/marea-distribution-profiles/create` }
    ]">
            ...
            <div class="max-w-6xl mx-auto w-full">
                ...
                <div class="rounded-xl border border-sidebar-border/70 dark:border-sidebar-border bg-card dark:bg-card p-6 mb-6">
                    ...
                    <p class="text-muted-foreground dark:text-muted-foreground">
                        Create a new financial distribution profile for mareas
                    </p>
                </div>
