...
const handlePdfDownload = (options: { type: 'month' | 'range'; month?: number; year?: number; startDate?: string; endDate?: string; transactionType?: string; enableColors?: boolean }) => {
    ...
    downloadTimeout = setTimeout(() => {
        ...
        if (options.type === 'month') {
            ...
            url = `/panel/${vesselId}/movimentations/history/${options.year}/${options.month}/download-pdf${queryString ? '?' + queryString : ''}`;
        } else {
            ...
            url = `/panel/${vesselId}/movimentations/download-pdf?${params.toString()}`;
        }
...
const confirmDelete = () => {
    ...
    const vesselId = getCurrentVesselId();
    const url = `/panel/${vesselId}/movimentations/${transactionToDelete.value.id}`;
    router.delete(url, {
        onSuccess: () => {
