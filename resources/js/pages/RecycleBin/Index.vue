    ...
    router.post(`/panel/${getCurrentVesselId()}/recycle-bin/${item.type}/${item.id}/restore`, {}, {
        onSuccess: () => {
            ...
            addNotification({
                type: 'success',
                title: 'Success',
                message: `${item.type_label} '${item.name}' has been restored successfully.`,
            });
        },
        onError: () => {
            isProcessing.value = false;
    ...
    router.delete(`/panel/${getCurrentVesselId()}/recycle-bin/${item.type}/${item.id}`, {
        onSuccess: () => {
            ...
            addNotification({
                type: 'success',
                title: 'Success',
                message: `${item.type_label} '${item.name}' has been permanently deleted.`,
            });
        },
        onError: () => {
            isProcessing.value = false;
    ...
    router.post(`/panel/${getCurrentVesselId()}/recycle-bin/empty`, {}, {
        onSuccess: () => {
            ...
            addNotification({
                type: 'success',
                title: 'Success',
                message: 'Recycle bin has been emptied successfully.',
            });
        },
        onError: () => {
            isProcessing.value = false;
            addNotification({
                type: 'error',
                title: 'Error',
                message: 'Failed to empty recycle bin. Please try again.',
            });
        },
    });
};
...
<template>
    ...
    <VesselLayout :breadcrumbs="[{ title: 'Recycle Bin', href: `/panel/${getCurrentVesselId()}/recycle-bin` }]">
        <div class="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
            <!-- Header Card -->
            <div class="rounded-xl border border-sidebar-border/70 dark:border-sidebar-border bg-card dark:bg-card p-6">
                <div class="flex items-center justify-between">
                    <div>
                        <h1 class="text-2xl font-semibold text-card-foreground dark:text-card-foreground">Recycle Bin</h1>
                ...
                <div class="mt-6 grid grid-cols-1 md:grid-cols-5 gap-4">
                    ...
                    <div class="p-4 rounded-lg bg-green-50 dark:bg-green-900/10 border border-green-200 dark:border-green-800">
                        <div class="text-sm font-medium text-green-800 dark:text-green-300 mb-1">Suppliers</div>
                    ...
                    <div class="p-4 rounded-lg bg-purple-50 dark:bg-purple-900/10 border border-purple-200 dark:border-purple-800">
                        <div class="text-sm font-medium text-purple-800 dark:text-purple-300 mb-1">Recurring Transactions</div>
            ...
            <div class="rounded-xl border border-sidebar-border/70 dark:border-sidebar-border bg-card dark:bg-card overflow-hidden">
                <div v-if="filteredItems.length === 0" class="px-6 py-12 text-center text-muted-foreground dark:text-muted-foreground">
                    ...
                    <p class="text-lg font-medium mb-2">No deleted items found</p>
                    <p class="text-sm">The recycle bin is empty or no items match your filters.</p>
                </div>
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <div class="mt-1 flex items-center gap-4 text-xs text-muted-foreground dark:text-muted-foreground">
                                        ...
                                        <span v-if="item.transaction_count !== undefined && item.transaction_count > 0" class="text-orange-600 dark:text-orange-400">
                                            {{ item.transaction_count }} transaction(s)
                                        </span>
                                    </div>
        ...
        <ConfirmationDialog
            ...
            :message="itemToRestore ? `Are you sure you want to restore ${itemToRestore.type_label.toLowerCase()} '${itemToRestore.name}'?` : ''"
        ...
        <ConfirmationDialog
            ...
            :message="itemToDelete ? `Are you sure you want to permanently delete ${itemToDelete.type_label.toLowerCase()} '${itemToDelete.name}'? This action cannot be undone.` : ''"
        ...
        <ConfirmationDialog
            ...
            description="This action cannot be undone. All items in the recycle bin will be permanently deleted."
            :message="`Are you sure you want to empty the recycle bin? This will permanently delete ${totalCount} item(s). This action cannot be undone.`"
