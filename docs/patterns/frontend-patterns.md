...
<template>
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        >
            <!-- Header Card -->
            <div
                class="rounded-xl border border-sidebar-border/70 bg-card p-6 dark:border-sidebar-border dark:bg-card"
            >
                <!-- Header content -->
            </div>
        ...
        >
            ...
            >
                <div class="flex items-center justify-between">
                    <div>
                        <h1
                            class="text-2xl font-semibold text-card-foreground dark:text-card-foreground"
                        >
                            {{ title }}
                        </h1>
                        <p
                    ...
                    <Link
                        :href="createUrl"
                        class="inline-flex items-center rounded-lg bg-primary px-4 py-2 font-medium text-primary-foreground transition-colors hover:bg-primary/90"
                        v-if="canCreate"
                    >
            ...
            <!-- Content Card -->
            <div
                class="overflow-hidden rounded-xl border border-sidebar-border/70 bg-card dark:border-sidebar-border dark:bg-card"
            >
                <TransactionList
                ...
                <div
                    ...
                    class="border-t border-border bg-card px-4 py-3 sm:px-6 dark:border-border dark:bg-card"
                >
                    <Pagination
...
<template>
    <div class="space-y-2">
        <Label
            :for="id"
            class="text-sm font-medium text-card-foreground dark:text-card-foreground"
            >{{ label }}</Label
        >
        <div class="relative">
            <Input
                ...
                :class="[
                    'w-full rounded-lg border border-input bg-background px-3 py-2 text-foreground placeholder:text-muted-foreground focus:border-transparent focus:ring-2 focus:ring-ring focus:outline-none dark:border-input dark:bg-background dark:text-foreground dark:placeholder:text-muted-foreground',
...
<template>
    <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-border dark:divide-border">
            <thead class="bg-muted/50 dark:bg-muted/50">
                <tr>
                    <th
                        ...
                        :key="column.key"
                        class="px-6 py-3 text-left text-xs font-medium tracking-wider text-muted-foreground uppercase dark:text-muted-foreground"
                        :class="column.class"
                    >
                        {{ column.label }}
                    </th>
                    <th
                        v-if="hasActions"
                        class="px-6 py-3 text-right text-xs font-medium tracking-wider text-muted-foreground uppercase dark:text-muted-foreground"
                    >
                        Actions
                    </th>
                </tr>
            ...
            <tbody
                class="divide-y divide-border bg-card dark:divide-border dark:bg-card"
            >
                <tr
                    v-for="item in data"
                    :key="getItemKey(item)"
                    class="transition-colors hover:bg-muted/50 dark:hover:bg-muted/50"
                >
                    <td
                        v-for="column in columns"
                        :key="column.key"
                        class="px-6 py-4 text-sm whitespace-nowrap text-card-foreground dark:text-card-foreground"
                    ...
                    >
                        <slot name="actions" :item="item">
                            <button
                                ...
                                class="mr-2 inline-flex items-center rounded-lg border border-border bg-secondary px-3 py-1 text-sm font-medium text-secondary-foreground transition-colors hover:bg-secondary/80 dark:border-border dark:text-secondary-foreground"
                            >
                                Edit
                            </button>
                            <button
                                ...
                                class="text-destructive transition-colors hover:text-destructive/80 dark:text-destructive dark:hover:text-destructive/80"
                            >
                                Delete
                            </button>
                        </slot>
...
## Dropdown Menus in Tables and Lists

### Overview

When implementing dropdown menus in table rows or list items, always use Vue's `Teleport` component to render the dropdown outside the container. This prevents the dropdown from being clipped by parent containers with `overflow-hidden` or `overflow-x-auto`.

### Pattern: Teleported Dropdown Menu
...
- Close dropdown on scroll, resize, and outside clicks
- Use high z-index (`z-[9999]`) to ensure visibility
...
<template>
    <div
        class="overflow-x-auto overflow-y-visible rounded-xl border border-sidebar-border/70 bg-card dark:border-sidebar-border dark:bg-card"
    >
        <!-- Table or List -->
        <div v-for="item in items" :key="item.id">
            <div class="dropdown-container relative">
                <button
                    ...
                    class="flex h-8 w-8 items-center justify-center rounded-full transition-colors hover:bg-muted dark:hover:bg-muted"
                >
        ...
        <Teleport to="body">
            ...
            >
                <div
                    class="w-48 rounded-lg border border-border bg-card shadow-lg dark:border-border dark:bg-card"
                >
                    <div class="py-1">
                        <button
                            @click="handleAction(item)"
                            class="flex w-full items-center px-4 py-2 text-sm text-card-foreground transition-colors hover:bg-muted dark:text-card-foreground dark:hover:bg-muted"
                        >
                            <Icon name="eye" class="mr-3 h-4 w-4" />
                            View Details
                        </button>
                    </div>
                </div>
...
1. **Container Overflow**: Change `overflow-hidden` to `overflow-x-auto overflow-y-visible` on the parent container to allow dropdowns to escape vertically while maintaining horizontal scrolling.

2. **Position Calculation**: Always calculate position from the button's `getBoundingClientRect()` to ensure accurate placement.

3. **Event Handling**: Use `@click.stop` on both the button and dropdown to prevent event bubbling.

4. **Cleanup**: Always remove event listeners in `onUnmounted` to prevent memory leaks.

5. **Z-Index**: Use `z-[9999]` or higher to ensure dropdown appears above all other content.

### Where This Pattern is Used

- ✅ `DataTable` component (`resources/js/components/ui/DataTable.vue`)
- ✅ Maintenances Index page (`resources/js/pages/Maintenances/Index.vue`)
- ✅ Mareas Index page (`resources/js/pages/Mareas/Index.vue`)
- ✅ Any page using the `DataTable` component automatically benefits from this pattern

### Alternative: Using DropdownMenu Component

For header dropdowns or simpler cases, use the existing `DropdownMenu` component which already uses `DropdownMenuPortal`:
...
export function useTransaction() {
    ...
    const vatAmount = computed(() => {
        ...
        const rate = vatRates.value.find(
            (r: any) => r.id === form.value.vat_rate_id,
        );
        ...
        <div class="space-y-6">
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-4">
                ...
                <SummaryCard
                    ...
                    currency="EUR"
                    :variant="
                        summary.net_balance >= 0 ? 'positive' : 'negative'
                    "
                />
                <SummaryCard
        ...
        <div class="mx-auto max-w-2xl">
            ...
            <form @submit.prevent="submitForm" class="space-y-6">
                <div class="space-y-6 rounded-lg bg-white p-6 shadow">
                    ...
                    <!-- VAT Amount Display -->
                    <div v-if="vatAmount > 0" class="rounded-lg bg-gray-50 p-4">
                        <div class="flex items-center justify-between">
...
const { notifications, processFlashMessages, removeNotification } =
    useNotifications();
...
export function usePermissions() {
    ...
    const currentVesselRole = computed(
        () => user.value?.vessel_role || 'viewer',
    );
...
<template>
    <div>
        ...
        <PermissionGate permission="vessels.create">
            <Button @click="openCreateModal"> Add Vessel </Button>
        </PermissionGate>

        <!-- Using composable directly -->
        <Button v-if="canCreate('vessels')" @click="openCreateModal">
            Add Vessel
        </Button>
...
<template>
    <div>
        ...
        <div class="flex space-x-2">
            ...
            <Button v-if="canEdit('vessels')" @click="editVessel(vessel)">
                Edit
            </Button>
            <Button
...
<template>
    <BaseModal
        ...
        :description="
            isEditing ? 'Update entity information' : 'Add a new entity'
        "
        size="lg"
...
<template>
    <div>
        <!-- Page content -->
        <Button @click="openCreateModal"> Add Entity </Button>

        <!-- Table with edit buttons -->
        <Button @click="openEditModal(entity)"> Edit </Button>
